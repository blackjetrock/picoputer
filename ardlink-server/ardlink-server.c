      ...
      if( frame_n == (frame_length+2) )
	{
	  ...
	  if( frame_length >= 6 )
		...
		case SP_READ_TAG:
		  ...
		  // We must send back an even number of bytes
		  int additional;
		  ...
		  send_byte(link, (read_length+additional) % 256);
		  packet_loop(link, done_on_ack);
		  send_byte(link, (read_length+additional) / 256);
		  packet_loop(link, done_on_ack);
		...
		case SP_WRITE_TAG:
		  ...
		  for(int i=0; i<write_length; i++)
		    {
		      printf("%02X ", frame[FROFF_WRITE_DATA+i]);
		    }
		  printf("\n");
		...
		case SP_GETS_TAG:
		  ...
		  // length
		  send_byte(link, (data_length+3) % 256);
		  packet_loop(link, done_on_ack);
		  send_byte(link, (data_length+3) / 256);
		  packet_loop(link, done_on_ack);
		  ...
		  if( data_length != 0 )
		    ...
		    {
		      send_byte(link, SP_ERROR);
		      packet_loop(link, done_on_ack);
		    }

		  // and send it
		  send_byte(link, (data_length) % 256);
		  packet_loop(link, done_on_ack);
		  send_byte(link, (data_length) / 256);
		  packet_loop(link, done_on_ack);
...
#if SP_DEBUG
		  printf("SP_GETS  Stream ID:%08X Length:%04X (%d) data length:%d\n", stream_id, read_length, read_length, data_length);
		...
		case SP_PUTS_TAG:
		  ...
		  for(int i=0; i<write_length; i++)
		    {
		      printf("%02X ", frame[FROFF_PUTS_DATA+i]);
		    }
		  printf("nl ");
		  printf("\n");
...
#if SP_DEBUG
		  printf("SP_GETKEY  Stream ID:%08X Length:%04X (%d)\n", stream_id, read_length, read_length);
